...
class Pawn(resource: Int, color: Color, tag: Char, coordinate: Coordinate) :
    ...
    override fun showPossibleMoves(show: Boolean, check: Boolean) {
        ...
        if(check) {
            isInCheck(Coordinate.fromString("${coordinate.name[0]}${coordinate.name[1].asInt + (1 * moveDir)}"))
            isInCheck(Coordinate.fromString("${coordinate.name[0] + 1}${coordinate.name[1].asInt + (1 * moveDir)}"))
            isInCheck(Coordinate.fromString("${coordinate.name[0] - 1}${coordinate.name[1].asInt + (1 * moveDir)}"))
        } else {
            if (isStartPos()) isLegalSquare(show, Coordinate.fromString("${coordinate.name[0]}${coordinate.name[1].asInt + (2 * moveDir)}"))
            isLegalSquare(show, Coordinate.fromString("${coordinate.name[0]}${coordinate.name[1].asInt + (1 * moveDir)}"))
            isLegalSquare(show, Coordinate.fromString("${coordinate.name[0] + 1}${coordinate.name[1].asInt + (1 * moveDir)}"))
            isLegalSquare(show, Coordinate.fromString("${coordinate.name[0] - 1}${coordinate.name[1].asInt + (1 * moveDir)}"))
